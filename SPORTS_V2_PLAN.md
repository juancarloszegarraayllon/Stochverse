...
- **C2c-c — request-time wiring of v2**: point `sports_feed_v2` at `registry.resolve_through_alias` instead of the existing `compute_fl_identity`/`kalshi_join` chain. Existing path stays accessible behind `?v=2` for the verification window. Riskiest piece, gets its own PR. Status: _planned_.
