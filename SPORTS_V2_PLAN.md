...
- **C2 — Phase C deferrals**: guarded fuzzy tier (third tier — fires only when same-comp same-date and exactly one unpaired FL fixture + one Kalshi record remain in the bucket), market-layer seeding (per-market and per-outcome alias rows so price chips become canonical-id-keyed), per_leg ticker handling, request-time wiring of v2 to the registry. Status: _planned_.
