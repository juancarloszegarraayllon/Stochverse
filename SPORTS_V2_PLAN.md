...
- **C2c-c2 — request-time promotion (next PR)**: once `/api/_debug/registry_diff` shows clean parity (`v2_only_pairings == 0` and `mixed_pairings == 0` across the in-season sports), add a user-visible flag (e.g. `?v=3`) on `sports_feed_v2` that routes through the registry path. v2 stays accessible behind `?v=2` for the verification window. Status: _planned_.
