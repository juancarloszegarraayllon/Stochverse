...
- **C2b — Phase C deferrals (still planned)**: market-layer seeding (per-market and per-outcome alias rows so price chips become canonical-id-keyed — required for Polymarket/OddsAPI multi-source price aggregation), per_leg ticker handling (tennis sets, esports maps as parameterized fixtures), request-time wiring of v2 to the registry (pointing `sports_feed_v2` at `resolve_through_alias` instead of the existing `kalshi_join` flow). Status: _planned_.
