"""Bootstrap Liga ACB (Spanish Basketball League) team coverage — Phase 2D.5-A.

Data-driven league bootstrap: Liga ACB teams identified via
asymmetric_anchor_failure resolver signal (Day-27 diagnostic).
~400 records/week resolving to review_queue because sp.teams has
no Basketball-sport canonical for Spanish league team names.

Inserts 18 Liga ACB teams + their alias coverage into sp.teams and
sp.team_aliases so future Liga ACB records auto-resolve via strict
tier instead of routing to review_queue.

Mirrors scripts/bootstrap_lmb.py structure with the same PR #200
...
Cross-sport collision notes:
  - Real Madrid Baloncesto + FC Barcelona Bàsquet canonicals
    intentionally distinct from soccer canonicals (Real Madrid CF,
    FC Barcelona) likely already in sp.teams under sport_id=soccer.
  - Bare "Real Madrid" + "Barcelona" aliases safe under sport_id
    partition (resolver/aliases.py:51,111). Day-22 architectural
    finding empirically validated.
  - "Madrid" bare alias INTENTIONALLY EXCLUDED — too generic, risks
    collision with future Madrid-area basketball clubs (Estudiantes).

Idempotent — re-running is a no-op for rows + aliases already present.

## Usage

    DATABASE_URL=<url> python scripts/bootstrap_acb.py
    DATABASE_URL=<url> python scripts/bootstrap_acb.py --dry-run
...
from scripts.liga_acb_seed import LIGA_ACB_ALIAS_SOURCE, LIGA_ACB_TEAMS_SEED  # noqa: E402
...
async def bootstrap(dry_run: bool) -> int:
    """Insert/update Liga ACB team + alias coverage. Returns process exit code."""
    log = get_logger("bootstrap.acb")
    started = time.monotonic()
    log.info("bootstrap.acb.start", dry_run=dry_run,
             manifest_size=len(LIGA_ACB_TEAMS_SEED))
    ...
    # INSERT 18 teams + 98 aliases into the wrong DB.
    allow_non_prod = (
    ...
    if not allow_non_prod:
        ...
        if rc != 0:
            ...
            return 3
        log.info("bootstrap.acb.pattern_d.ok",
                 current_database=current_db,
                 expected_db_name=expected_db_name,
                 expected_db_host=expected_db_host)
    else:
        log.info("bootstrap.acb.pattern_d.bypass",
    ...
    async with async_session() as session:
        ...
        log.info("bootstrap.acb.sport_resolved",
        ...
        log.info("bootstrap.acb.existing_teams_loaded",
        ...
        for idx, (canonical_name, country_code, aliases, _notes) in enumerate(
            ...
            if not normalized:
                ...
                log.warning(
                    "bootstrap.acb.empty_normalized",
        ...
        log.info(
            "bootstrap.acb.teams_classified",
        ...
        if affected_team_ids:
            alias_rows = (await session.execute(
                text(
                    ...
                    source=LIGA_ACB_ALIAS_SOURCE,
                ),
            )).all()
            existing_aliases = {
                (r.team_id, r.alias_normalized) for r in alias_rows
            }
        log.info("bootstrap.acb.existing_aliases_loaded",
        ...
        for idx, (_cname, _ccode, aliases, _notes) in enumerate(
            ...
            for alias_raw in aliases:
                ...
                if not alias_normalized:
                    ...
                    log.warning(
                        "bootstrap.acb.alias_empty_normalized",
                ...
                if alias_normalized in all_aliases_for_source:
                    ...
                    log.warning(
                        "bootstrap.acb.alias_global_conflict",
                        alias=alias_raw,
        ...
        log.info(
            "bootstrap.acb.aliases_classified",
        ...
        if dry_run:
            log.info(
                "bootstrap.acb.dry_run_skipping_writes",
        ...
        print(f"  Manifest entries:               {len(LIGA_ACB_TEAMS_SEED):>4}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Bootstrap Liga ACB (Spanish Basketball League) team coverage "
