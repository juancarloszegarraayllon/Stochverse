"""Bootstrap Turkish Basketbol Süper Ligi (BSL) team coverage —
Phase 2D.5-A workstream #5.
...
query). ~234 records/7d resolving to review_queue.

Inserts 16 Turkish BSL teams + their alias coverage into sp.teams and
sp.team_aliases so future BSL records auto-resolve via strict tier.

Mirrors scripts/bootstrap_israeli_bsl.py structure with the same
PR #200 alias-safety fix.

NEW METHODOLOGY DIMENSIONS THIS WORKSTREAM (per scope-doc):
  - F2 empirical-coverage discipline: bare-name aliases INCLUDED for
    5 football-overlap teams (sport_id partition validated Day-22)
  - Diacritic empirical verification: ş/ç/ü/ğ collapse via NFD;
    `ı` (dotless i) does NOT decompose, requires explicit ı + i pairs
  - Canonical-name fragmentation pattern: 2 dormant phantoms accepted
    (Karşıyaka legacy, Turk Telekom legacy) — manifest uses Wikipedia
    canonical, legacy stubs unchanged

BACKFILL predictions (3 manifest canonicals expected to match Phase
2A.5 legacy stubs on apply):
  - Anadolu Efes (legacy id ca2f4866-c4ac-4a26-976f-d54401ce8c1d)
  - Bursaspor Basketbol (legacy id 85c6d6bf-8ffb-4309-b0aa-9ba3d146ad4c)
  - Tofaş (legacy id 7f3d7ec1-c48f-48cf-8b8f-089faec3fc53,
    NFD normalize: Tofaş → tofas matches legacy Tofas → tofas)

Idempotent — re-running is a no-op for rows + aliases already present.

## Usage

    DATABASE_URL=<url> python scripts/bootstrap_turkish_bsl.py
    DATABASE_URL=<url> python scripts/bootstrap_turkish_bsl.py --dry-run
...
from scripts.turkish_bsl_seed import (  # noqa: E402
...
async def bootstrap(dry_run: bool) -> int:
    """Insert/update Turkish BSL team + alias coverage. Returns exit code."""
    log = get_logger("bootstrap.turkish_bsl")
    started = time.monotonic()
    log.info("bootstrap.turkish_bsl.start", dry_run=dry_run,
             manifest_size=len(TURKISH_BSL_TEAMS_SEED))
    ...
    if not allow_non_prod:
        ...
        if rc != 0:
            ...
            return 3
        log.info("bootstrap.turkish_bsl.pattern_d.ok",
                 current_database=current_db,
                 expected_db_name=expected_db_name,
                 expected_db_host=expected_db_host)
    else:
        log.info("bootstrap.turkish_bsl.pattern_d.bypass",
    ...
    async with async_session() as session:
        ...
        log.info("bootstrap.turkish_bsl.sport_resolved",
        ...
        log.info("bootstrap.turkish_bsl.existing_teams_loaded",
        ...
        for idx, (canonical_name, country_code, aliases, _notes) in enumerate(
            ...
            if not normalized:
                ...
                log.warning(
                    "bootstrap.turkish_bsl.empty_normalized",
        ...
        log.info(
            "bootstrap.turkish_bsl.teams_classified",
        ...
        if affected_team_ids:
            alias_rows = (await session.execute(
                text(
                    ...
                    source=TURKISH_BSL_ALIAS_SOURCE,
                ),
            )).all()
            existing_aliases = {
                (r.team_id, r.alias_normalized) for r in alias_rows
            }
        log.info("bootstrap.turkish_bsl.existing_aliases_loaded",
        ...
        for idx, (_cname, _ccode, aliases, _notes) in enumerate(
            ...
            for alias_raw in aliases:
                ...
                if not alias_normalized:
                    ...
                    log.warning(
                        "bootstrap.turkish_bsl.alias_empty_normalized",
                ...
                if alias_normalized in all_aliases_for_source:
                    ...
                    log.warning(
                        "bootstrap.turkish_bsl.alias_global_conflict",
                        alias=alias_raw,
        ...
        log.info(
            "bootstrap.turkish_bsl.aliases_classified",
        ...
        if dry_run:
            log.info(
                "bootstrap.turkish_bsl.dry_run_skipping_writes",
        ...
        log.info(
            "bootstrap.turkish_bsl.complete",
            dry_run=dry_run,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Bootstrap Turkish BSL (Basketbol Süper Ligi) team "
