"""Bootstrap EuroLeague gap-fill + ABA League team coverage —
Phase 2D.5-A workstreams #8 + #9 (combined).

Data-driven league bootstrap: EuroLeague + ABA League teams identified via
asymmetric_anchor_failure resolver signal (Day-35 discovery
query). EuroLeague gap-fill ~15/7d residual + ABA League ~90-100/7d.

Inserts 24 EuroLeague+ABA teams (4 INSERTs + 20 BACKFILLs from
Phase 2A.5 legacy stubs) + their alias coverage into sp.teams and
sp.team_aliases so future EuroLeague + ABA records auto-resolve via strict tier.

Mirrors scripts/bootstrap_turkish_bsl.py structure with the same
PR #200 alias-safety fix.

DORMANT PHANTOM RISK (5 phantoms identified pre-apply per
euroleague_aba_seed.py docstring): Iraklis, Kolossos Rodou, Maroussi,
Peristeri, Promitheas. Amendment #22 pre-apply audit MANDATORY
before wet apply.
...
## Usage
    ...
    DATABASE_URL=<url> python scripts/bootstrap_euroleague_aba.py
    DATABASE_URL=<url> python scripts/bootstrap_euroleague_aba.py --dry-run
...
from scripts.euroleague_aba_seed import (  # noqa: E402
...
async def bootstrap(dry_run: bool) -> int:
    """Insert/update EuroLeague + ABA team + alias coverage. Returns exit code."""
    log = get_logger("bootstrap.euroleague_aba")
    started = time.monotonic()
    log.info("bootstrap.euroleague_aba.start", dry_run=dry_run,
             manifest_size=len(EUROLEAGUE_ABA_TEAMS_SEED))
    ...
    if not allow_non_prod:
        ...
        if rc != 0:
            ...
            return 3
        log.info("bootstrap.euroleague_aba.pattern_d.ok",
                 current_database=current_db,
                 expected_db_name=expected_db_name,
                 expected_db_host=expected_db_host)
    else:
        log.info("bootstrap.euroleague_aba.pattern_d.bypass",
    ...
    async with async_session() as session:
        ...
        log.info("bootstrap.euroleague_aba.sport_resolved",
        ...
        log.info("bootstrap.euroleague_aba.existing_teams_loaded",
        ...
        for idx, (canonical_name, country_code, aliases, _notes) in enumerate(
            ...
            if not normalized:
                ...
                log.warning(
                    "bootstrap.euroleague_aba.empty_normalized",
        ...
        log.info(
            "bootstrap.euroleague_aba.teams_classified",
        ...
        if affected_team_ids:
            alias_rows = (await session.execute(
                text(
                    ...
                    source=EUROLEAGUE_ABA_ALIAS_SOURCE,
                ),
            )).all()
            existing_aliases = {
                (r.team_id, r.alias_normalized) for r in alias_rows
            }
        log.info("bootstrap.euroleague_aba.existing_aliases_loaded",
        ...
        for idx, (_cname, _ccode, aliases, _notes) in enumerate(
            ...
            for alias_raw in aliases:
                ...
                if not alias_normalized:
                    ...
                    log.warning(
                        "bootstrap.euroleague_aba.alias_empty_normalized",
                ...
                if alias_normalized in all_aliases_for_source:
                    ...
                    log.warning(
                        "bootstrap.euroleague_aba.alias_global_conflict",
                        alias=alias_raw,
        ...
        log.info(
            "bootstrap.euroleague_aba.aliases_classified",
        ...
        if dry_run:
            log.info(
                "bootstrap.euroleague_aba.dry_run_skipping_writes",
        ...
        log.info(
            "bootstrap.euroleague_aba.complete",
            dry_run=dry_run,
        ...
        print(f"\nEuroLeague + ABA bootstrap "
              f"{'dry-run' if dry_run else 'complete'} in {elapsed:.1f}s:")
        print(f"  Manifest entries:               {len(EUROLEAGUE_ABA_TEAMS_SEED):>4}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Bootstrap EuroLeague gap-fill + ABA League team "
                    "coverage into sp.teams + sp.team_aliases. "
                    "Idempotent.",
    )
    parser.add_argument(
