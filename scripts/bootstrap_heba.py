"""Bootstrap Greek HEBA A1 (Basket League) team coverage —
Phase 2D.5-A workstream #6.

Data-driven league bootstrap: Greek HEBA A1 teams identified via
asymmetric_anchor_failure resolver signal (Day-32 afternoon
discovery query). ~50-70 records/7d resolving to review_queue
(playoffs-only window).

Inserts 13 HEBA teams (4 INSERTs + 9 BACKFILLs from Phase 2A.5
legacy stubs) + their alias coverage into sp.teams and
sp.team_aliases so future HEBA records auto-resolve via strict tier.
...
heba_seed.py docstring): Iraklis, Kolossos Rodou, Maroussi,
...
## Usage
    ...
    DATABASE_URL=<url> python scripts/bootstrap_heba.py
    DATABASE_URL=<url> python scripts/bootstrap_heba.py --dry-run
...
from scripts.heba_seed import (  # noqa: E402
    HEBA_ALIAS_SOURCE,
    HEBA_TEAMS_SEED,
)
...
async def bootstrap(dry_run: bool) -> int:
    """Insert/update HEBA team + alias coverage. Returns exit code."""
    log = get_logger("bootstrap.heba")
    started = time.monotonic()
    log.info("bootstrap.heba.start", dry_run=dry_run,
             manifest_size=len(HEBA_TEAMS_SEED))
    ...
    if not allow_non_prod:
        ...
        if rc != 0:
            ...
            return 3
        log.info("bootstrap.heba.pattern_d.ok",
                 current_database=current_db,
                 expected_db_name=expected_db_name,
                 expected_db_host=expected_db_host)
    else:
        log.info("bootstrap.heba.pattern_d.bypass",
    ...
    async with async_session() as session:
        ...
        log.info("bootstrap.heba.sport_resolved",
        ...
        log.info("bootstrap.heba.existing_teams_loaded",
        ...
        for idx, (canonical_name, country_code, aliases, _notes) in enumerate(
            ...
            if not normalized:
                ...
                log.warning(
                    "bootstrap.heba.empty_normalized",
        ...
        log.info(
            "bootstrap.heba.teams_classified",
        ...
        if affected_team_ids:
            alias_rows = (await session.execute(
                text(
                    ...
                    source=HEBA_ALIAS_SOURCE,
                ),
            )).all()
            existing_aliases = {
                (r.team_id, r.alias_normalized) for r in alias_rows
            }
        log.info("bootstrap.heba.existing_aliases_loaded",
        ...
        for idx, (_cname, _ccode, aliases, _notes) in enumerate(
            ...
            for alias_raw in aliases:
                ...
                if not alias_normalized:
                    ...
                    log.warning(
                        "bootstrap.heba.alias_empty_normalized",
                ...
                if alias_normalized in all_aliases_for_source:
                    ...
                    log.warning(
                        "bootstrap.heba.alias_global_conflict",
                        alias=alias_raw,
        ...
        log.info(
            "bootstrap.heba.aliases_classified",
        ...
        if dry_run:
            log.info(
                "bootstrap.heba.dry_run_skipping_writes",
        ...
        print(f"\nGreek HEBA A1 bootstrap "
              f"{'dry-run' if dry_run else 'complete'} in {elapsed:.1f}s:")
        print(f"  Manifest entries:               {len(HEBA_TEAMS_SEED):>4}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Bootstrap Greek HEBA A1 (Basket League) team "
