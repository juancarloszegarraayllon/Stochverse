"""Bootstrap Russian VTB United League team coverage —
Phase 2D.5-A workstream #7.

Data-driven league bootstrap: VTB United League teams identified
via asymmetric_anchor_failure resolver signal (Day-34 discovery).
~230+ records/7d resolving to review_queue.

Inserts 11 VTB teams (5 INSERTs + 6 BACKFILLs from Phase 2A.5
legacy stubs) + alias coverage. Mirrors scripts/bootstrap_heba.py
structure with the same PR #200 alias-safety fix.
...
## Usage
    ...
    DATABASE_URL=<url> python scripts/bootstrap_vtb.py
    DATABASE_URL=<url> python scripts/bootstrap_vtb.py --dry-run
...
from scripts.vtb_seed import (  # noqa: E402
    VTB_ALIAS_SOURCE,
    VTB_TEAMS_SEED,
)
...
async def bootstrap(dry_run: bool) -> int:
    """Insert/update VTB team + alias coverage. Returns exit code."""
    log = get_logger("bootstrap.vtb")
    started = time.monotonic()
    log.info("bootstrap.vtb.start", dry_run=dry_run,
             manifest_size=len(VTB_TEAMS_SEED))
    ...
    if not allow_non_prod:
        ...
        if rc != 0:
            ...
            return 3
        log.info("bootstrap.vtb.pattern_d.ok",
                 current_database=current_db,
                 expected_db_name=expected_db_name,
                 expected_db_host=expected_db_host)
    else:
        log.info("bootstrap.vtb.pattern_d.bypass",
    ...
    async with async_session() as session:
        ...
        log.info("bootstrap.vtb.sport_resolved",
        ...
        log.info("bootstrap.vtb.existing_teams_loaded",
        ...
        for idx, (canonical_name, country_code, aliases, _notes) in enumerate(
            ...
            if not normalized:
                ...
                log.warning(
                    "bootstrap.vtb.empty_normalized",
        ...
        log.info(
            "bootstrap.vtb.teams_classified",
        ...
        if affected_team_ids:
            alias_rows = (await session.execute(
                text(
                    ...
                    source=VTB_ALIAS_SOURCE,
                ),
            )).all()
            existing_aliases = {
                (r.team_id, r.alias_normalized) for r in alias_rows
            }
        log.info("bootstrap.vtb.existing_aliases_loaded",
        ...
        for idx, (_cname, _ccode, aliases, _notes) in enumerate(
            ...
            for alias_raw in aliases:
                ...
                if not alias_normalized:
                    ...
                    log.warning(
                        "bootstrap.vtb.alias_empty_normalized",
                ...
                if alias_normalized in all_aliases_for_source:
                    ...
                    log.warning(
                        "bootstrap.vtb.alias_global_conflict",
                        alias=alias_raw,
        ...
        log.info(
            "bootstrap.vtb.aliases_classified",
        ...
        if dry_run:
            log.info(
                "bootstrap.vtb.dry_run_skipping_writes",
        ...
        print(f"\nRussian VTB United League bootstrap "
              f"{'dry-run' if dry_run else 'complete'} in {elapsed:.1f}s:")
        print(f"  Manifest entries:               {len(VTB_TEAMS_SEED):>4}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Bootstrap Russian VTB United League team coverage "
