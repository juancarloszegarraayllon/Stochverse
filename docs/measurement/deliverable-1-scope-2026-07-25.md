...
**Expected-noise pattern in `both_pair_different`** (added 2026-07-29 post-Run-5 task #28 triage): observed ~1 record/day at current volumes (single-record baseline; revisit as v4 coverage grows), v2's date-blind identity-parse will occasionally misjoin an fl_event to a structurally-unpairable Kalshi ticker family (mention markets, prop markets not yet in `_OUTRIGHT_SERIES_PREFIXES` at `kalshi_identity.py:101`). Because v4 doesn't extract-refuse these upfront but the downstream resolver correctly doesn't link them to the fl_event's fixture, they land in `both_pair_different` rather than `v4_extraction_excluded`. Sample and verify: if the disagreement is v2 pairing an fl_event to a Kalshi ticker matching a known-unpairable family (`KXMLBMENTION-*` etc.), treat as v2 defect not v4 defect. Fix belongs in v4's `_OUTRIGHT_SERIES_PREFIXES`, not in the diff's exclusion builder — see task #29. **Threshold-setting**: subtract this noise floor before reading the bucket count against Item 7's threshold.
...
- **D2/D1 window semantics — RESOLVED 2026-07-29 (task #25 closed)**. Pre-registered concern was that the 03:00 UTC cron's `< window_end` predicate would exclude still-active rows (`last_seen_at` bumped past `window_end` by PR #260's ~30s ingestion pass). Day-58 first cron read showed the OPPOSITE direction: fixture_linked GREW from Run 4's 70 to 97 (+39%). Code trace at `scripts/daily_diff.py:1602-1605` explains: `daily_diff()` defaults `window_end = datetime.now(timezone.utc)` at invocation and `window_start = window_end - 24h`. Both cron and manual runs use the SAME rolling-24h-ending-at-invocation semantics — active rows have `last_seen_at ~= NOW()` which sits inside the window (SQL is `< window_end` where window_end IS NOW()). The pre-registered concern assumed a midnight-aligned `window_end` — a mental model, not what the code produces. **Cross-report comparison discipline**: windows anchor to invocation time (rolling 24h ending NOW), so comparisons must mind the anchor offset between cron reports (all anchored 03:00 UTC) and any manual runs (anchored at whatever wall-clock the operator invoked). Direct day-over-day cron-to-cron comparisons are apples-to-apples; mixed cron+manual comparisons need offset normalization.
