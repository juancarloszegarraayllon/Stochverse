...
Status: design doc rev1.2. Locked through sub-PR #3 (PR #123). Rev1.2 (2026-05-12) corrects the anchor-failed `fail_reason` enumeration in §Q6 — earlier drafts listed `anchor_score_below_floor` (doesn't exist in the code) and `deferred_to_2d` (non-terminal; the record continues to fuzzy tier). The actual terminal anchor-failed family is `alias_no_team_resemblance`, `fuzzy_no_team_resemblance`, `alias_no_existing_fixture`, `fuzzy_no_existing_fixture`. Rev1.1 was the pre-implementation lock; rev1.0 was the initial draft. Pivot context (from rev1.0): 2D.5 paused per the 2026-05-10 production spot-check finding — the review queue is structurally accumulating into an unactionable state because operators have no UI to triage it.
...
- New routes: `GET /admin/anchor-failed`, `GET /admin/anchor-failed/<provider>/<provider_record_id>`. No approve/reject — anchor_failed records have no candidates to approve. Detail view shows the provider record, the matcher's `fail_reason`, and any near-miss candidates the matcher considered. **Rev1.2 fail_reason enumeration:** the anchor-failed family is exactly `alias_no_team_resemblance` (alias-tier final, no fuzzy run), `fuzzy_no_team_resemblance` (fuzzy-tier final after alias deferred), `alias_no_existing_fixture` (matched teams, no fixture exists), `fuzzy_no_existing_fixture` (fuzzy version of the same). Earlier drafts listed `anchor_score_below_floor` and `deferred_to_2d`; the former doesn't exist in the code and the latter is non-terminal (record continues to fuzzy tier per `resolver/matcher.py:471`). Ingestion-level failures (`sport_not_classified`, `kickoff_at_missing`, `structural_normalize_failed`, `home_and_away_same_team`, `kalshi_competition_unresolvable`) are NOT in the anchor-failed family — they belong to a separate ingestion-visibility surface, tracked separately if operator need surfaces.
